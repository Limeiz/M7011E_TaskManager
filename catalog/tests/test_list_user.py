...
class ListUserTestCase(APITestCase):
    def setUp(self):
        ...
        self.reg_user = User.objects.create_user(username='testUser', password='testpass123')
        ...
        self.list = List.objects.create(list_name='Test List', slug='test-list')
        self.list.assigned_tasks.add(self.task)
        self.list.assigned_users.add(self.reg_user)

    def test_get_list_as_regular_user(self):
        response = self.client.get(reverse('list_user_details', args=[str(self.list.slug)]))
    ...
    def test_update_list_as_regular_user(self):
        ...
        response = self.client.patch(reverse('list_user_details', args=[str(self.list.slug)]),
                                     {'list_name': new_list_name})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['list_name'], new_list_name)

    def test_delete_list_as_regular_user(self):
        response = self.client.delete(reverse('list_user_details', args=[str(self.list.slug)]))
