...
class ListAdminTestCase(APITestCase):
    ...
    def test_get_list_as_admin(self):
        response = self.client.get(reverse('list_admin_details', args=[str(self.list.slug)]))
    ...
    def test_update_list_as_admin(self):
        ...
        response = self.client.patch(reverse('list_admin_details', args=[str(self.list.slug)]),
                                     {'list_name': new_list_name})
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(response.data['list_name'], new_list_name)

    def test_delete_list_as_admin(self):
        response = self.client.delete(reverse('list_admin_details', args=[str(self.list.slug)]))
