...
class TaskAdminTestCase(APITestCase):
    def setUp(self):
        ...
        self.task = Task.objects.create(task_name='Test Task', priority='l', status='t', slug='test-task', assignee=self.reg_user)
        ...
        self.other_users_task = Task.objects.create(task_name='Secret Task', priority='l', status='t',
                                                    slug='secret-task', assignee=self.other_user)
    ...
    def test_get_secret_task(self):
        response = self.client.get(reverse('task_user_details', args=[str(self.other_users_task.slug)]))
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_update_task(self):
        new_task_name = 'Updated Task'
        response = self.client.patch(reverse('task_user_details', args=[str(self.task.slug)]),
    ...
    def test_update_secret_task(self):
        ...
        response = self.client.patch(reverse('task_user_details', args=[str(self.other_users_task.slug)]),
                                     {'task_name': new_task_name})
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)

    def test_delete_task(self):
        response = self.client.delete(reverse('task_user_details', args=[str(self.task.slug)]))
        self.assertEqual(response.status_code, status.HTTP_204_NO_CONTENT)
        self.assertFalse(Task.objects.filter(slug=self.task.slug).exists())

    def test_delete_secret_task(self):
        response = self.client.delete(reverse('task_user_details', args=[str(self.other_users_task.slug)]))
        self.assertEqual(response.status_code, status.HTTP_403_FORBIDDEN)
        self.assertTrue(Task.objects.filter(slug=self.other_users_task.slug).exists())
